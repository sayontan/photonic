...
;( function( $, window, document, undefined )
{
	...
	$.fn.imageLightbox = function( options )
	{
		var options	   = $.extend(
			...
			{
				// Changed by Sayontan because 500px returns no extension for the image...
				//return $( element ).prop( 'tagName' ).toLowerCase() == 'a' && ( new RegExp( '\.(' + options.allowedTypes + ')$', 'i' ) ).test( $( element ).attr( 'href' ) );
				//return $( element ).prop( 'tagName' ).toLowerCase() == 'a';
				return true;
			},

			// Added by Sayontan to support dynamically modified elements. $.eq() fails to find a match in such cases -->
				...
				setTimeout( function()
				{
					image = $( '<img ' + options.selector + ' />' )
						...
						.load( function()
						{
							...
							{
								/* Changed by Sayontan to support dynamically modified elements. $.eq() fails to find a match in such cases */
					...
					image.on( hasPointers ? 'pointerup MSPointerUp' : 'click', function( e )
					{
						...
						{
							/* Changed by Sayontan to address Chrome capturing the <code>pointermove</code> event */
							//if( !wasTouched( e.originalEvent ) || options.quitOnImgClick ) return true;
