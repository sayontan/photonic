...
abstract class Photonic_OAuth1_Processor extends Photonic_Processor {
	...
	protected function generate_signature($api_call = null, $api_args = NULL, $method = null, $oauth_token = array()) {
		...
		$end_point = $this->provider == 'smug' ? remove_query_arg(array('_expand','_config'), $api_call) : $api_call;
	 ...
	 * @return null
	 */
	function get_request_token($oauth_callback = null, $set_cookie = true) {
...
/*		if ($this->provider == 'flickr' || $this->provider == 'smug') {
			$method = 'GET';
		}
		else {
			$method = 'POST';
...
//			$parameters['method'] = $this->request_token_URL();
		...
		if (isset($token['oauth_token']) && isset($token['oauth_token_secret']) && $set_cookie) {
	 ...
	 * @return
	 */
	function get_access_token($request_token, $set_cookie = true) {
/*		if ($this->provider == 'flickr' || $this->provider == 'smug') {
			$method = 'GET';
		}
		else {
		...
		if (isset($token['oauth_token']) && isset($token['oauth_token_secret']) && $set_cookie) {
			...
			else if (isset($request_token['oauth_verifier']) && $this->provider == 'smug') {
				setcookie('photonic-'.$secret.'-oauth-verifier', $request_token['oauth_verifier'], time() + 365 * 60 * 60 * 24, COOKIEPATH);
			}
		}
	...
	public function sign_call($api_method, $method, $parameters) {
		...
		if (isset($this->token) && isset($this->token_secret)) {
			$token = array('oauth_token' => $this->token, 'oauth_token_secret' => $this->token_secret);
		}
		else if (${$allow_oauth}) {
...
/*			else if (isset($this->token) && isset($this->token_secret)) {
				$token = array('oauth_token' => $this->token, 'oauth_token_secret' => $this->token_secret);
			}*/
		}
	...
	public function set_oauth_done() {
		if (!empty($this->token) && !empty($this->token_secret)) {
			$token_response = $this->check_access_token(array('oauth_token' => $this->token, 'oauth_token_secret' => $this->token_secret));
